...
这是一个给 Codex 使用的中文短视频获客诊断 skill。
...
- 一份清楚的账号底盘：卖什么、卖给谁、为什么现在需要、下一步让用户做什么。
- 一套精准用户处境：不再用年龄、性别、城市这类标签糊弄自己。
- 一个可维护的选题库：每条选题都有需求、解决方向、服务人群、内容类型和状态。
- 一套内容诊断标准：知道一条内容是跑偏、能发测试，还是只需要改一层。
- 一个案例库和素材库：把客户语言、反馈、截图、服务过程沉淀成信任资产。
...
- 发内容前：丢一条选题、脚本、产品介绍、服务介绍或免费资料介绍，让它判断框架是否跑偏。
- 想选题时：让它基于你的账号底盘、用户处境和客户语言建设精准选题库。
- 有客户反馈时：把咨询记录、评论、私信、截图、成交异议放进来，判断是否值得进入案例库或素材库。
- 发完内容后：把播放、完播、评论、私信、咨询、成交情况丢进来，做发布后复盘。
